...
function loadInnerAudioContext (url) {
    return new Promise((resolve, reject) => {
        const nativeAudio = $global.__globalAdapter.createInnerAudioContext();
        ...
        function fail () {
            clearEvent();
            clearTimeout(timer);
            reject('failed to load innerAudioContext: ' + $global.err);
        }
...
function loadAudioPlayer (url, options, onComplete) {
    $global.cc.AudioPlayer.load(url).then(player => {
        const audioMeta = {
            player,
            url,
...
const downloadCCON = (url, options, onComplete) => {
    downloadJson(url, options, (err, json) => {
        ...
        const cconPreface = $global.cc.internal.parseCCONJson(json);
        const chunkPromises = Promise.all(cconPreface.chunks.map((chunk) => new Promise((resolve, reject) => {
            downloadArrayBuffer(`${$global.cc.path.mainFileName(url)}${chunk}`, {}, (errChunk, chunkBuffer) => {
                if (errChunk) {
                    reject(errChunk);
                } else {
        ...
        chunkPromises.then((chunks) => {
            const ccon = new $global.cc.internal.CCON(cconPreface.document, chunks);
...
const downloadCCONB = (url, options, onComplete) => {
    downloadArrayBuffer(url, options, (err, arrayBuffer) => {
        ...
        try {
            const ccon = $global.cc.internal.decodeCCONBinary(new Uint8Array(arrayBuffer));
            onComplete(null, ccon);
        } catch (err) {
            onComplete(err);
        }
...
function downloadBundle (nameOrUrl, options, onComplete) {
    ...
    let version = options.version || $global.cc.assetManager.downloader.bundleVers[bundleName];
